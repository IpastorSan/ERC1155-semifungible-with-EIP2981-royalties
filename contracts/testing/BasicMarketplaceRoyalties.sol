...
contract BasicMarketplaceRoyalties is Ownable, ERC165 {
    ...
    function buyExactMatchNative(uint256 tokenId, uint256 amount, bytes memory data) external payable {
        ...
        token.safeTransferFrom(_list.seller, msg.sender, tokenId, amount, data);
    ...
    function getToken() internal view returns(CarbonBits) {
        CarbonBits token = CarbonBits(payable(nft));
        return token;
    }
